...
| 🙋 **高危操作审批** | `delete_file` 等默认需人工确认。审批属于**会话**而非发起端：终端与所有浏览器标签页同时看到同一条待审，任一处应答即生效（[`DualApprovalCallback`](src/callback/dual_approval.rs)）。支持按参数动态判定是否需审批、"本会话总是允许/拒绝"的记忆、附带拒绝理由；**无人应答时该轮会挂起并落盘**，进程重启后仍可 `/resume` 继续 |
...
| ✅ **工程质量** | 无 `.unwrap()` 生产路径、零硬编码密钥、528 个测试、clippy 全绿（原生 + wasm） |
...
| `/resume` | `/continue` | 继续被挂起的那一轮（会重新询问待批准的操作） | ✅ |
| `/discard` | - | 放弃被挂起的那一轮，已完成的部分保留在历史里 | ✅ |
...
`/reset` 清掉的不只是对话历史，还包括本会话内"总是允许/拒绝某工具"的记忆（见上方「高危操作审批」），以及尚未处理的挂起轮次——挂起的那一轮持有的正是刚被清空的那段对话，留着它等于允许把清空过的会话再续回去。`--fresh` 同理。
...
- **没人回答不算回答**。无人应答（超时，默认 300 秒，见 `AGENT_APPROVAL_TIMEOUT_SECS`）、没有任何前端在监听、收到待审的界面直接关掉——这三种情况都**不会**被当成"拒绝"，而是把这一轮**挂起并落盘**，等人回来再处理（见下方「挂起与恢复」）。调用本身当然没有执行，安全上与拒绝等价；区别在于不会替人编造一个他没给出的答复。这也不只是语义洁癖：告诉模型"用户拒绝了"，它会针对一个没人提出的反对意见继续推理若干轮，那是实打实的 token。
- **超时仍然是必需的**。一轮对话会持有该会话的轮次锁直到结束，没有上界的等待会把所有界面一起卡死。超时的作用是**释放这把锁**，而不是替人做决定。
...
没人应答的那一轮会被存下来（默认目录 `.agent/approvals`，可用 `AGENT_CLI_APPROVAL_DIR` 覆盖），**进程重启后依然在**：

- 终端启动时会提示 `⏸ 本会话有一轮在等待审批时被暂停（待批准: delete_file）`；网页刷新后会看到一张「已暂停」卡片（`GET /api/suspended`）。
- `/resume` 继续这一轮：**不重放已完成的工具调用**，只把待批准的那些重新问一遍——所以恢复看起来就是审批卡片再次出现，答复方式与第一次完全一样。
- `/discard` 放弃这一轮：已经跑完的工具结果保留在历史里，待批准的那些记为"未获批准"，对话可以正常往下走。
- 两端通用：网页上「已暂停」卡片的两个按钮，走的就是 `/resume` 和 `/discard`。终端产生的挂起可以在浏览器里恢复，反之亦然。

几个刻意的设计：

- **进程没了也不会丢**。这一轮的状态在每个工具轮次开始*之前*就写盘了，所以即使被 `kill -9`、崩溃或断电，下次启动依然找得到。三种可捕获的信号（`Ctrl-C`、关闭终端窗口的 `SIGHUP`、`kill` 的 `SIGTERM`）会先把这一轮优雅挂起再退出——`Ctrl-C` 也因此成了「这个审批我待会儿再说」的快捷方式，不用干等满超时。
- **被强制中断的一轮不会被「恢复」，而是如实收尾**。一轮里的调用是并发的，进程突然消失时哪些已经生效无从得知，重跑会把副作用做第二遍。所以这类记录在下次启动时自动收尾为「是否生效未知」，历史保持可用，对话能继续——实测模型收到这条记录后会主动去核实，而不是臆断。
- **挂起的那一轮不写进会话历史**。它的 transcript 是半截的（有工具调用、没有结果），写进去会让整个会话再也发不出去。它连同那一轮的提问一起存在挂起状态里，恢复时一并回来。
- **一个会话同时只能有一个挂起的轮次**，此时新消息会被拦下并提示先 `/resume` 或 `/discard`。挂起状态里有一段还没进会话存储的历史，此时另起一轮会从残缺的 transcript 分叉，之后再恢复就会把两段分歧的历史拼到一起。
- **换了模型 / 改了 system prompt / 工具集变了就不给恢复**，并明确说明变的是什么。存下来的那一轮不会因此被丢掉——改回去还能接着恢复，也可以 `/discard` 主动放弃。
...
`cli` 的 Web 服务器会把这个 `dist/` 目录当静态资源伺服（`AGENT_CLI_WEB_DIST_DIR` 可覆盖路径）；开发前端时也可以单独 `trunk serve` 起热重载的开发服务器，接口请求转发到 `cli` 的 `/api/*` 路由。页面加载后会依次拉取 `GET /api/history` 展示已有会话、`GET /api/approvals` 补上此刻仍在等待的审批、`GET /api/suspended` 补上被挂起还没处理的那一轮，随后打开一条持久的 `GET /api/stream` 长连接（浏览器原生 `EventSource`），终端和浏览器发起的每一轮对话都会广播到这条连接上——因此终端里打的字也会实时出现在网页里，反过来也一样；`POST /api/chat` 只负责提交这一轮的输入本身。展示内容包括 token 流、工具调用/结果时间线，遇到高危操作会弹出确认卡片，点击后调用 `POST /api/approve/{id}` 提交决策，决策结果也会广播给所有打开的标签页。

> 后两个拉取都不是冗余的，而且各自覆盖不同的情况。`GET /api/stream` 背后的广播只推给"当时已经在监听"的订阅者且不重放，而审批与挂起**都有意不写进对话历史**（前者是对某次调用的闸门，后者的 transcript 是半截的）。`GET /api/approvals` 管的是「这一轮还在跑、正卡在审批上」——少了它，此时刷新页面就再也看不到、也无法应答那条正阻塞着它的待审。`GET /api/suspended` 管的是「这一轮已经停了」——那时连正在跑的对话都没有，没有任何东西会推送过来，少了它，新开的页面会显示一个看起来已经结束的对话，完全看不出还有一轮在等人处理。
...
| `AGENT_CLI_APPROVAL_DIR` | `.agent/approvals` | 挂起轮次的落盘目录（与会话目录分开：两者生命周期相反，清空历史不该顺手丢掉一个待批准的操作） |
...
| `AGENT_APPROVAL_TIMEOUT_SECS` | `300` | 审批等待人工决策的上限。超时**不按拒绝处理**，而是挂起该轮并落盘（见「挂起与恢复」）；这个上界的作用是释放会话的轮次锁 |
...
│   ├── fingerprint.rs  RunFingerprint：挂起的那一轮由什么配置产生，恢复前据此校验
│   ├── approval_store.rs  挂起轮次的存取（与 session 分开，生命周期相反）
...
四种审批响应里唯一没做的一种：允许人工修正模型给出的参数后再放行，而不只是批准/拒绝二选一。

**暂不做，因为现有能力已经覆盖了大部分场景**——带理由拒绝（`n: 路径应该是 /tmp/x 而不是 /`）会让模型自己改参数重试，路径长一步，但不需要任何新机制，也不引入下面这些问题。等出现「模型反复改不对、只能人来填」的真实场景再做。

真要做时，已核实过的影响面（详见 [`docs/approval-hitl-plan.md`](docs/approval-hitl-plan.md) 4.4）：

- **`ToolCallDecision` 加一个 `ProceedWith` 变体几乎无成本**。四个 before-hook 只返回它、从不 `match` 它，全仓唯一的穷尽 `match` 在 `execute_tool_calls` 一处。
- **难点不在返回类型，在 transcript 的一致性**。`record_tool_calls` 在审批之前就把原始参数写进了历史，改写之后两者就对不上了。而 `build_messages` 会把历史里的参数原样发回给模型——于是模型看到的是「自己发的原始参数 + 一个成功结果」，会认定原参数被执行了。这是正确性问题，不只是审计问题。
- **倾向的方案**：`record_tool_calls` 照旧，决策阶段把改写值收集起来，在 `execute_tool_calls` 末尾拿回可变借用时就地回填，且**只回填真正执行了的调用**（被拒绝的记原始，被挂起的靠 `SuspendedToolCall` 带走改写值）。这样时序与并发都不变，而「轮内中间态从不落盘、从不发给模型」这条已有的结构性保证正好兜住回填前的窗口。
- **已知的坑**：给历史追加一条"修正项"是最差的做法——仓里有三处遍历 `ToolCall`，对同 id 重复项分别取「最早 / 最后 / 第一个」，语义互相冲突，而且 `build_messages` 会让同一个 `tool_call_id` 在一个请求里出现两次，多数 provider 直接拒绝。
- **三条附带约束**：改写值必须进 `SuspendedToolCall`（否则挂起再恢复会静默丢失改写）；`edit` 与"本会话总是允许"互斥（下次调用参数不同，记忆没有意义）；改写后是否告知模型要单独决策，若告知则说明文字必须带来源标注（理由同上文的 `User denied execution of ...` 前缀）。
